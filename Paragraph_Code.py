...
def load_text():
    return random.choice(Paragraphs.paragraphs).strip()
    # strip() removes the '\0\ at the end of the string


def display_text(stdscr, target, current, penalty_rate, actual_wpm, wpm=0):
    stdscr.addstr(target)
    stdscr.addstr(6, 0, f"WPM : {wpm}", curses.color_pair(3))
    stdscr.addstr(7, 0, f"Penalty : {penalty_rate}", curses.color_pair(3))
    stdscr.addstr(8, 0, f"Actual WPM : {actual_wpm}", curses.color_pair(3))
